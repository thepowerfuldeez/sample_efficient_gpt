...
run with uv run torchrun --nproc_per_node 4 edu_filter_ds.py 
"""
...
if __name__ == "__main__":
    ...
    filepath = "/mnt/harddrive/datasets/cosmopedia-v2-textbook-7b/data/"
    output_dir = "/mnt/harddrive/datasets/cosmopedia-v2-textbook-7b/data_filtered"
    Path(output_dir).mkdir(exist_ok=True)
    
    # process only first 3B
    parquet_files = list(Path(filepath).glob("*.parquet"))
    # parquet_files = parquet_files[:24]
