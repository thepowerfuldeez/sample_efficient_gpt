...
class Nemotron(Task):
    ...
    def __init__(self, **kwargs):
        ...
        self.ds = concatenate_datasets(
            load_dataset("nvidia/Nemotron-Post-Training-Dataset-v2", split=["math", "code", "stem", "chat"])
        ).shuffle(seed=42)
    ...
    def get_example(self, index):
        ...
        # assert len(rest_messages) >= 2, "Nemotron messages must have at least 2 messages"
        # for i, message in enumerate(rest_messages):
        #     # user and assistant alternate as user,assistant,user,assistant,...
        #     expected_role = "user" if i % 2 == 0 else "assistant"
        #     if message["role"] != expected_role:
        #         messages = [first_message] + rest_messages[:i]
        #         break
